...
Le hero est sur fond sombre (`--encre`), les autres sections restent claires.
Deux fonds coexistent selon la largeur d'écran :

**À partir de 768 px — shader WebGL.** Moteur repris de `shader-source.tsx`
(fichier de référence à la racine, à ne pas importer tel quel : seule la
logique WebGL est reprise, jamais sa mise en page, ses props ni ses textes).
Filaments recolorés en safran `#D98C1F` et olive `#3E8A66`. Corrections
obligatoires du fichier source : `<style jsx>` remplacé par du CSS classique,
classes `WebGLRenderer` et `PointerHandler` sorties du hook, `useRef<number |
null>(null)`, gestionnaire de pointeur supprimé.

**Sous 768 px — canvas 2D.** Les 7 courbes gaussiennes de `reference.html`,
recolorées pour le fond sombre. Pas de WebGL sur mobile, sans exception.

Contraintes non négociables, valables pour les deux :

- `devicePixelRatio` plafonné à 1 pour le shader, 1.5 pour le canvas 2D
...
- repli sur le canvas 2D si le contexte `webgl2` est indisponible
- le canvas reste derrière le contenu : canvas en `z-index: 1`, conteneur du
  contenu en `position: relative; z-index: 2`
- composant client isolé, chargé en `dynamic(..., { ssr: false })`
- aucune bibliothèque d'animation tierce (Framer Motion, GSAP, three.js…)

Si Lighthouse mobile passe sous 90, le shader saute — la promesse
« impeccable sur téléphone » prime sur l'effet visuel.
